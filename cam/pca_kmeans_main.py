...
class ClusterScoreCAM(BaseCAM):
    ...
    Bổ sung bước PCA để giảm chiều dựa trên số thành phần chính trước khi clustering.
    ...
    Args:
        ...
        pca_components: số thành phần chính để giữ lại (int); None để không dùng PCA
    """
    def __init__(
        self,
        model_dict,
    ...
    def forward(self, input, class_idx=None, retain_graph=False):
        ...
        use_pca = isinstance(self.pca_components, int) and 0 < self.pca_components < flat_maps.shape[1]
        if use_pca:
            pca = PCA(n_components=self.pca_components, random_state=0)
            reduced = pca.fit_transform(flat_maps)  # (nc, pca_components)
        else:
