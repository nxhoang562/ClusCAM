...
class DiffCAM(BaseCAM):
    """
    Score-CAM với clustering, sử dụng trực tiếp độ lệch logit làm trọng số và không zero-out.
    ...
    def forward(self, input, class_idx=None, retain_graph=False):
        ...
        # Lưu lại kết quả clustering
        self.rep_maps = rep_maps
        self.base_score = base_score

        # 5) Tính độ lệch logit làm trọng số
        diffs = torch.zeros(self.K, device=activations.device)
        with torch.no_grad():
            for k in range(self.K):
                mask = rep_maps[k:k+1].unsqueeze(0)  # (1,1,h,w)
        ...
        # 6) Dùng trực tiếp độ lệch làm trọng số
        weights = diffs

        # 7) Kết hợp saliency map
        sal = torch.zeros(1, 1, h, w, device=activations.device)
        for k in range(self.K):
            sal += weights[k] * rep_maps[k:k+1].unsqueeze(0)

        # 8) Post-process + normalize (không qua ReLU)
        mn, mx = sal.min(), sal.max()
        if mn == mx:
            # Trường hợp constant map, raise lỗi như trước
            raise RuntimeError("Constant saliency map: min and max are equal, cannot normalize")
        sal = (sal - mn) / (mx - mn)
